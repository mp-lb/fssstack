...
To create the first super admin, set the user's public metadata to this in Clerk dashboard:
...
Expose a `ctx.isAdmin` boolean on the tRPC context that's true for `"admin" | "super_admin"` so that procedures can easily check.